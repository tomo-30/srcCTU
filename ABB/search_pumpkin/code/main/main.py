...
# ==== ピン閾値（GUIで変更可能）====
DEFAULT_THRESH_ANY_MM = 10.0   # ② いずれかのピンが <= この値
DEFAULT_THRESH_ALL_MM = 15.0  # ③ すべてのピンが <= この値
...
# ==== DEBUG/TRACE ====
DEBUG_TRACE = True            # ターミナル出力ON/OFF
TRACE_TO_CSV = True           # CSVにも保存したい場合 True
TRACE_DIR = os.path.join(BASE_DIR, "log")
os.makedirs(TRACE_DIR, exist_ok=True)
TRACE_PATH = None

def _sync_and_get_cart(client, *, retries=3, sleep_s=0.03):
    """
    通信の取り違え対策：
      1) 受信バッファflush
      2) ping×2でサーバ応答を“現在位置”に揃える
      3) get_cart を呼ぶ（失敗したら数回リトライ）
    """
    for t in range(retries):
        try:
            ...
            print(f"[TRACE] _sync_and_get_cart retry {t+1}/{retries} after error: {e}")
            time.sleep(0.05 + 0.02*t)
    # すべて失敗したら例外で上へ
    raise RuntimeError("get_cart failed after sync/retries")
...
def _dbg(msg):
    if DEBUG_TRACE:
        print(f"[TRACE]{_now()} {msg}")

def _quat_dot(q, p):  # q=(qx,qy,qz,qw)
    return q[0]*p[0] + q[1]*p[1] + q[2]*p[2] + q[3]*p[3]

def _ang_between_quats_deg(q, p):
    # 角度=2*acos(|dot|) 度
    d = max(-1.0, min(1.0, _quat_dot(q, p)))
    import math
    return 2.0*math.degrees(math.acos(abs(d)))

def _safe_rpy_to_quat(r,p,y):
    q = rpy_deg_to_quat(r,p,y)
    # 正規化（念のため）
    n = max(1e-12, math.sqrt(sum(v*v for v in q)))
    return [q[0]/n, q[1]/n, q[2]/n, q[3]/n]

def _trace_csv_write(row: dict):
    global TRACE_PATH
    if not TRACE_TO_CSV:
        return
    try:
        ...
        if TRACE_PATH is None:
            # 開始時に作る
            fname = time.strftime("trace_%Y%m%d_%H%M%S.csv")
        ...
        print(f"[TRACE][CSV] write error: {e}")
...
def _unwrap_deg(curr: float, prev: float) -> float:
    # curr を prev に“近い”表現に丸める（±180 を跨がない）
    d = curr - prev
    d = (d + 180.0) % 360.0 - 180.0
    return prev + d

def _unwrap_rpy(curr_rpy, prev_rpy):
    cr, cp, cy = curr_rpy
    pr, pp, py = prev_rpy
    return (_unwrap_deg(cr, pr), _unwrap_deg(cp, pp), _unwrap_deg(cy, py))
...
class PipelineMover:
    ...
    def __init__(self, client,
        ...
        self._prev_rpy_sent = None  # 直前に送ったRPY（unwrap後）
    ...
    def prelude(self, tool_txyz=(0,0,0), tool_rpy=(0,0,0), use_identity_wobj=True):
        ...
        # 現在姿勢を初期値として記憶（unwrapの基準）
        ax, ay, az, qx, qy, qz, qw = self.c.get_cart()
        self._prev_rpy_sent = quat_to_rpy_deg(qx, qy, qz, qw)
    ...
    def run(self, segments: List[PoseRPY], check_abort: Optional[Callable[[], bool]] = None):
        ...
        for k in range(prefill):
            ...
            s = segments[k]
            rpy_raw = (s.roll, s.pitch, s.yaw)
            rpy_unw = _unwrap_rpy(rpy_raw, self._prev_rpy_sent)
            q_prev = _safe_rpy_to_quat(*self._prev_rpy_sent)
            q_send = _safe_rpy_to_quat(*rpy_unw)
            ang = _ang_between_quats_deg(q_send, q_prev)

            _dbg(f"[PRELOAD] idx={k}/{N-1} SEND_ONLY "
                 f"XYZ=({s.x:.2f},{s.y:.2f},{s.z:.2f}) "
                 f"RPY_raw=({rpy_raw[0]:.3f},{rpy_raw[1]:.3f},{rpy_raw[2]:.3f}) "
                 f"RPY_unw=({rpy_unw[0]:{'.3f'}},{rpy_unw[1]:.3f},{rpy_unw[2]:.3f}) "
                 f"ANG_TO_PREV={ang:.3f}deg")
            _trace_csv_write({
                "t": _now(), "phase": "PRELOAD", "label": "", "ix": k, "cmd": "send_only",
                "x": s.x, "y": s.y, "z": s.z,
                "roll": rpy_raw[0], "pitch": rpy_raw[1], "yaw": rpy_raw[2],
                "roll_unw": rpy_unw[0], "pitch_unw": rpy_unw[1], "yaw_unw": rpy_unw[2],
                "ang_to_prev_deg": ang,
                "zone": f"{self.zone_blend}", "v_tcp": self.v_tcp, "v_ori": self.v_ori,
            })

            self._prev_rpy_sent = rpy_unw
            self.c.moveL_send_only(s.x, s.y, s.z, rpy_unw, tiny_pause=0.0)
        ...
        while i < N:
            ...
            # 直前の実機姿勢（ACK前）
            cur_x,cur_y,cur_z,qx,qy,qz,qw = _sync_and_get_cart(self.c)
            cur_rpy = quat_to_rpy_deg(qx,qy,qz,qw)

            # Zone 切替
            if i == N - 1:
                self.zs.set_zone_if_changed(*self.zone_stop, force=True, context="final-stop")
                zone_str = f"{self.zone_stop}"
                phase = "FINAL_ACK"
            else:
                self.zs.set_zone_if_changed(*self.zone_blend, context="blend")
                zone_str = f"{self.zone_blend}"
                phase = "PIPE_ACK"

            _dbg(f"[{phase}] idx={i}/{N-1} "
                 f"XYZ=({s.x:.2f},{s.y:.2f},{s.z:.2f}) "
                 f"RPY_raw=({rpy_raw[0]:.3f},{rpy_raw[1]:.3f},{rpy_raw[2]:.3f}) "
                 f"RPY_unw=({rpy_unw[0]:.3f},{rpy_unw[1]:.3f},{rpy_unw[2]:.3f}) "
                 f"ANG_TO_PREV={ang:.3f}deg ZONE={zone_str} "
                 f"CUR=({cur_x:.2f},{cur_y:.2f},{cur_z:.2f},{cur_rpy[0]:.2f},{cur_rpy[1]:.2f},{cur_rpy[2]:.2f})")

            self.c.moveL_ack(s.x, s.y, s.z, rpy_unw)

            # ACK後の実機姿勢
            post_x,post_y,post_z,qx2,qy2,qz2,qw2 = _sync_and_get_cart(self.c)
            post_rpy = quat_to_rpy_deg(qx2,qy2,qz2,qw2)
            _dbg(f"[{phase}] POST "
                 f"POSE=({post_x:.2f},{post_y:.2f},{post_z:.2f},"
                 f"{post_rpy[0]:.2f},{post_rpy[1]:.2f},{post_rpy[2]:.2f})")

            _trace_csv_write({
                "t": _now(), "phase": phase, "label": "", "ix": i, "cmd": "ack",
                "x": s.x, "y": s.y, "z": s.z,
                "roll": rpy_raw[0], "pitch": rpy_raw[1], "yaw": rpy_raw[2],
                "roll_unw": rpy_unw[0], "pitch_unw": rpy_unw[1], "yaw_unw": rpy_unw[2],
                "ang_to_prev_deg": ang,
                "zone": zone_str, "v_tcp": self.v_tcp, "v_ori": self.v_ori,
                "cur_x": cur_x, "cur_y": cur_y, "cur_z": cur_z,
                "cur_roll": cur_rpy[0], "cur_pitch": cur_rpy[1], "cur_yaw": cur_rpy[2],
                "post_x": post_x, "post_y": post_y, "post_z": post_z,
                "post_roll": post_rpy[0], "post_pitch": post_rpy[1], "post_yaw": post_rpy[2],
            })
            ...
            if i < N:
                if check_abort and check_abort(): return
                n = segments[i]
                rpy_raw_n = (n.roll, n.pitch, n.yaw)
                rpy_unw_n = _unwrap_rpy(rpy_raw_n, self._prev_rpy_sent)
                q_send_n = _safe_rpy_to_quat(*rpy_unw_n)
                ang_n = _ang_between_quats_deg(q_send_n, _safe_rpy_to_quat(*self._prev_rpy_sent))

                _dbg(f"[PIPE_PRELOAD] next idx={i}/{N-1} SEND_ONLY "
                     f"XYZ=({n.x:.2f},{n.y:.2f},{n.z:.2f}) "
                     f"RPY_raw=({rpy_raw_n[0]:.3f},{rpy_raw_n[1]:.3f},{rpy_raw_n[2]:.3f}) "
                     f"RPY_unw=({rpy_unw_n[0]:.3f},{rpy_unw_n[1]:.3f},{rpy_unw_n[2]:.3f}) "
                     f"ANG_TO_PREV={ang_n:.3f}deg")
                _trace_csv_write({
                    "t": _now(), "phase": "PIPE_PRELOAD", "label": "", "ix": i, "cmd": "send_only",
                    "x": n.x, "y": n.y, "z": n.z,
                    "roll": rpy_raw_n[0], "pitch": rpy_raw_n[1], "yaw": rpy_raw_n[2],
                    "roll_unw": rpy_unw_n[0], "pitch_unw": rpy_unw_n[1], "yaw_unw": rpy_unw_n[2],
                    "ang_to_prev_deg": ang_n,
                    "zone": f"{self.zone_blend}", "v_tcp": self.v_tcp, "v_ori": self.v_ori,
                })

                self._prev_rpy_sent = rpy_unw_n
                self.c.moveL_send_only(n.x, n.y, n.z, rpy_unw_n, tiny_pause=0.0)
# =======================================================================
...
class App(tk.Tk):
    ...
    def _run_single_target_with_arduino(self, label: str, target_xyzrpy, target_id: int):
        ...
        _dbg(f"[GO] label={label} going_to_B={going_to_B} target="
             f"({tx:.2f},{ty:.2f},{tz:.2f},{rr:.2f},{pp:.2f},{yw:.2f}) "
             f"step={step_mm} lookahead={lookahead} final_fine={final_fine} pause_s={pause_sec}")
            ...
            if going_to_B:
                ...
                if any(v <= thresh_any for v in vals) or all(v <= thresh_all for v in vals):
                    ...
                    if retreat_A_pose is not None:
                        ...
                        _dbg(f"[RETREAT_TO_A] A=({ax2:.2f},{ay2:.2f},{az2:.2f},{ar2:.2f},{ap2:.2f},{ayw2:.2f})")
        ...
        if going_to_B:
            ...
            r_act,p_act,y_act = quat_to_rpy_deg(qx,qy,qz,qw)
            actual_xyzrpy = (ax,ay,az,r_act,p_act,y_act)
            _dbg(f"[ARRIVE_B] at {label} ACT=({ax:.2f},{ay:.2f},{az:.2f},{r_act:.2f},{p_act:.2f},{y_act:.2f}) "
                 f"pins={pins_snapshot}")
